      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-8">
        ...
        {tab !== "gangs" && (
          <div className="border border-[#91897C]/30 bg-[#2f2922]">
            ...
            {/* Column headers */}
            <div className="grid grid-cols-[36px_1fr_64px_80px] gap-4 px-4 py-2.5 border-b border-[#91897C]/20 font-mono text-[10px] uppercase tracking-[0.14em] text-[#91897C]">
              <span>#</span>
              <span>Player</span>
            ...
            {loading && (
              <div className="py-14 text-center">
                <p className="font-mono text-xs uppercase tracking-widest text-[#91897C] animate-pulse">Loading…</p>
              </div>
            )}
            ...
            {!loading && sortedPlayers.map((p, i) => {
              ...
              return (
                <div
                  key={p.address}
                  className={`grid grid-cols-[36px_1fr_64px_80px] items-center gap-4 px-4 py-3 border-b border-[#91897C]/10 last:border-0 ${isMe ? "bg-[#EEF083]/5" : ""}`}
                >
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-mono text-xs font-black truncate" style={{ color: isMe ? "#EEF083" : "#d8d4a1" }}>
                        {isMe ? truncatedAddress : truncAddr(p.address)}
                        {isMe && <span className="ml-1.5 font-mono text-[8px] border border-[#EEF083]/40 px-1">YOU</span>}
                      </p>
                      {(() => {
                        const g = guilds.find((g) => g.members.includes(p.address));
                        return g ? (
                          <p className="font-mono text-[9px] text-[#91897C] truncate">[{g.tag}] {g.name}</p>
                        ) : null;
                      })()}
                    </div>
                  </div>

                  <span className="text-right font-mono text-sm text-[#91897C]">{p.matches_played}</span>
                  <span className="text-right font-mono text-sm font-black text-[#EEF083]">
            ...
            {!loading && addr && myRank === -1 && (
              <div className="border-t border-[#91897C]/30">
                ...
                <div className="grid grid-cols-[36px_1fr_64px_80px] items-center gap-4 px-4 py-3 bg-[#EEF083]/5">
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {myGuild && <p className="font-mono text-[9px] text-[#91897C]">[{myGuild.tag}] {myGuild.name}</p>}
                    </div>
                  </div>
                  <span className="text-right font-mono text-sm text-[#91897C]">{myRow?.matches_played ?? 0}</span>
                  <span className="text-right font-mono text-sm font-black text-[#91897C]">
        ...
        {tab === "gangs" && (
          <div className="border border-[#91897C]/30 bg-[#2f2922]">
            <div className="grid grid-cols-[36px_1fr_64px_90px] gap-4 px-4 py-2.5 border-b border-[#91897C]/20 font-mono text-[10px] uppercase tracking-[0.14em] text-[#91897C]">
              <span>#</span>
              <span>Gang</span>
            ...
            {guilds.map((g, i) => {
              ...
              return (
                <div
                  key={g.id}
                  className={`grid grid-cols-[36px_1fr_64px_90px] items-center gap-4 px-4 py-3 border-b border-[#91897C]/10 last:border-0 ${isMe ? "bg-[#EEF083]/5" : ""}`}
                >
                  <span className="font-mono text-sm font-black" style={{
                    color: i === 0 ? "#EEF083" : i === 1 ? "#aaa" : i === 2 ? "#cd7f32" : "#91897C"
                  }}>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isMe && <span className="font-mono text-[8px] text-[#EEF083] border border-[#EEF083]/40 px-1 shrink-0">YOU</span>}
                    </div>
